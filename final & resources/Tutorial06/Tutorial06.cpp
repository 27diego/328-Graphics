...
void Render()
{
	...
	g_World = XMMatrixRotationY( t*box_rotate );
	g_World *= XMMatrixRotationZ(t);
	g_World *= XMMatrixScaling(box_scale, 1.0f, 1.0f);//fix
	



	g_View *= XMMatrixTranslation(x_offset, 0.0f, z_offset);
	g_View *= XMMatrixRotationY(c_rotate);

    // Setup our lighting parameters
    XMFLOAT4 vLightDirs[4] =
    {
    ...
    XMFLOAT4 vLightColors[4] =
    {
        XMFLOAT4( 255.0f, 255.0f, 0.0f, 1.0f ),
	...
	XMMATRIX mRotate2 = XMMatrixRotationZ(2.0f * t);
...
void OnKeyDown(HWND hwnd, UINT vk, BOOL fDown, int cRepeat, UINT flags)
{
	...
	case 88://x
		counter++;
		OutputDebugString(L"else working\n");
		if (counter % 2 == 0)
		{
			box_scale = 1.0f;
		}
